...
(function () {
  ...
  // create a modest amount of snow
  for (let i=0;i<6;i++) setTimeout(createSnow, i*350);
  setInterval(createSnow, 700);

  // --- load catalogue.json ---
  function fetchCatalogue() {
    // try to fetch catalogue.json — error gives a clear console message
    return fetch('catalogue.json', {cache: "no-store"}).then(res => {
      if (!res.ok) throw new Error('catalogue.json fetch failed: ' + res.status + ' ' + res.statusText);
      return res.json();
    });
  }
  ...
  function makeImgWithFallback(srcFromJson, alt) {
    ...
    img.alt = alt || '';
    img.loading = 'lazy';
    // Use absolute fallback: if srcFromJson starts with "./" keep it, otherwise prefer relative
    img.src = srcFromJson;
    img.onerror = function () {
      try {
        ...
        if (name && img.src.indexOf(name) === -1) {
          ...
          // last fallback: try the images folder names or hide
